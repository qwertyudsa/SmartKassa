...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    // Получение рецептов, где используется ингредиент
    fun getRecipesUsingIngredient(ingredientId: String): List<Recipe> {
        val recipes = mutableListOf<Recipe>()
        val db = this.readableDatabase
        val query = """
        SELECT r.*, p.$COLUMN_NAME as product_name, i.$COLUMN_NAME as ingredient_name,
               p.$COLUMN_UNIT as product_unit, i.$COLUMN_UNIT as ingredient_unit
        FROM $TABLE_RECIPES r
        JOIN $TABLE_PRODUCTS p ON r.$COLUMN_PRODUCT_ID_FK = p.$COLUMN_PRODUCT_ID
        JOIN $TABLE_PRODUCTS i ON r.$COLUMN_INGREDIENT_ID_FK = i.$COLUMN_PRODUCT_ID
        WHERE r.$COLUMN_INGREDIENT_ID_FK = ?
        ORDER BY p.$COLUMN_NAME
    """.trimIndent()

        val cursor = db.rawQuery(query, arrayOf(ingredientId))
    ...
    // В классе DatabaseHelper добавляем:

    fun clearAllTables(): Boolean {
        val db = this.writableDatabase
        db.beginTransaction()

        try {
            // Удаляем все данные из таблиц (но не сами таблицы)
            ...
            // Не удаляем пользователей, чтобы сохранить логин

            db.setTransactionSuccessful()
            return true

        } catch (e: Exception) {
            Log.e("DatabaseHelper", "Ошибка очистки таблиц: ${e.message}")
            return false
        } finally {
            db.endTransaction()
        }
